...
public class SnakeGame extends Applet implements Runnable, KeyListener {
	...
	public void paint(Graphics g){
		...
		if(!gameOver){
			snake.draw(gfx); // draw the offscreen graphics
			token.draw(gfx);
		}
		else{
			gfx.setColor(Color.red);
			gfx.drawString("Game Over", 180, 150);
			gfx.drawString("Score: " + token.getScore(), 180, 300);
		}
	...
	public void keyPressed(KeyEvent e) {
		...
		if(e.getKeyCode() == KeyEvent.VK_UP){
			// if snake is not going down we will allow the snake to go up and not going side way
			if(snake.getYDir() != 1){
				snake.setYDir(-1);
				snake.setXDir(0);
